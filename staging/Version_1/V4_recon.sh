...
SCRIPT_DIR="../migration/Version_1/Feature_3_reconcille"
DB_NAME="erp_staging"
...
# Run Startup SQL
docker exec -i "$CONTAINER_NAME" psql -U "$DB_USER" -d "$DB_NAME" < "$SCRIPT_DIR/Financial.sql"
if [ $? -eq 0 ]; then
    echo "[$(date)] Staging Financial SQL alright!" >> "$LOG_FILE"
else
    echo "[$(date)] Staging Financial SQL failed!" >> "$LOG_FILE"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    echo "[$(date)] Staging Recon SQL alright!" >> "$LOG_FILE"
else
